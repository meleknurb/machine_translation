...
def preprocess_input_sentence(sentence):
  ...
  for token in tokens:
    if token in input_features_dict:
      input_seq.append(input_features_dict[token])
    else:
      input_seq.append(input_features_dict.get('<UNK>', 0))

  input_array = np.zeros((1, len(input_seq), len(input_features_dict)), dtype='float32')
  for t, idx in enumerate(input_seq):
    input_array[0, t, idx] = 1.
  return input_array
...
for sentence in test_sentences:
  test_input = preprocess_input_sentence(sentence)
  translation = decode_sequence(test_input)
